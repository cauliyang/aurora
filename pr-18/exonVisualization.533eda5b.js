(0,globalThis.parcelRequire4cf7.register)("iNwrO",function(t,e){function o(t,e,n,r=null){let i=Math.min(...t.map(t=>t.start)),s=Math.max(...t.map(t=>t.end)),l=s-i+1,d=e.clientWidth-60-30,c=d3.select(e).append("svg").attr("width",d+60+30).attr("height",275).attr("class","exon-svg").append("g").attr("transform","translate(60,45)");r?c.append("text").attr("x",d/2).attr("y",-5).attr("text-anchor","middle").attr("class","chromosome-info text-header").text(`Total Exons: ${t.length} | Chromosome ${r.chrom}${r.strand?`, Strand: ${r.strand}`:""}`):c.append("text").attr("x",d/2).attr("y",-5).attr("text-anchor","middle").attr("class","text-header").style("font-size","14px").style("fill","#666").text(`Total Exons: ${t.length}`);let p=c.append("defs"),x=d3.scaleLinear().domain([i,s]).range([0,d]),m=d3.axisBottom(x).tickFormat(t=>t.toLocaleString()).ticks(Math.min(10,d/100)),h=c.append("g").attr("class","x-axis").attr("transform","translate(0,170)").call(m);h.selectAll("text").style("font-size","10px").style("font-weight","bold"),h.selectAll("line").style("stroke","#ccc"),h.selectAll("path").style("stroke","#ccc").style("stroke-width","2px"),c.append("text").attr("x",d/2).attr("y",220).style("text-anchor","middle").style("font-size","12px").style("font-weight","bold").text("Genomic Position"),c.append("g").attr("class","grid-lines").selectAll("line").data(x.ticks(10)).enter().append("line").attr("x1",t=>x(t)).attr("y1",0).attr("x2",t=>x(t)).attr("y2",170).attr("stroke","#eee").attr("stroke-width",1);let g=t.reduce((t,e)=>t+e.length,0),u=[];for(let e=0;e<t.length-1;e++)t[e].end<t[e+1].start&&u.push({start:t[e].end+1,end:t[e+1].start-1,length:t[e+1].start-t[e].end-1,index:e});c.append("line").attr("x1",x(i)).attr("y1",90).attr("x2",x(s)).attr("y2",90).attr("stroke","#ccc").attr("stroke-width",3),p.append("pattern").attr("id","intron-pattern").attr("patternUnits","userSpaceOnUse").attr("width",6).attr("height",6).append("path").attr("d","M-1,1 l2,-2 M0,6 l6,-6 M5,7 l2,-2").attr("stroke","#999").attr("stroke-width",1),c.selectAll(".intron").data(u).enter().append("line").attr("class","intron").attr("x1",t=>x(t.start)).attr("y1",90).attr("x2",t=>x(t.start)).attr("y2",90).attr("stroke","#999").attr("stroke-width",3).attr("stroke-dasharray","5,5").transition().duration(1e3).delay((t,e)=>100*e).attr("x2",t=>x(t.end));let b=[d3.interpolateBlues,d3.interpolateGreens,d3.interpolateOranges,d3.interpolatePurples],v=b[t.length%b.length],f=d3.scaleSequential().domain([0,t.length-1]).interpolator(v),y=c.selectAll(".exon-group").data(t).enter().append("g").attr("class","exon-group").attr("transform",t=>`translate(${x(t.start)}, 70)`);y.append("rect").attr("class","exon").attr("x",0).attr("y",0).attr("width",0).attr("height",0).attr("fill",(t,e)=>f(e)).attr("stroke","#333").attr("stroke-width",2).attr("rx",6).attr("ry",6).transition().duration(800).delay((t,e)=>100*e).attr("height",40).transition().duration(400).attr("width",t=>Math.max(6,x(t.end)-x(t.start))),y.append("text").attr("class","exon-label").attr("x",t=>Math.max(3,(x(t.end)-x(t.start))/2)).attr("y",25).attr("text-anchor","middle").attr("fill","white").attr("font-weight","bold").attr("font-size","12px").attr("pointer-events","none").style("opacity",0).text((t,e)=>e+1),y.on("mouseover",function(e,o){let n=t.indexOf(o);d3.select(this).select("rect").transition().duration(150).attr("stroke-width",2).attr("stroke","#ff7f0e"),d3.select(this).select(".exon-label").transition().duration(150).style("opacity",1);let a=d3.select("#tooltip");if(a.empty())return void console.warn("Tooltip element not found");a.transition().duration(200).style("opacity",1),a.html(`
    ...
    `}(),e.insertBefore(o,e.firstChild),new XMLSerializer().serializeToString(e)}(o),n=new Blob([t],{type:"image/svg+xml;charset=utf-8"}),r=document.createElement("a");r.href=URL.createObjectURL(n),r.download=`${e}.svg`,r.style.display="none",document.body.appendChild(r),r.click(),setTimeout(()=>{document.body.removeChild(r),URL.revokeObjectURL(r.href),a("success","SVG exported successfully!")},100)}catch(t){console.error("Error exporting SVG:",t),a("error",`Failed to export SVG: ${t.message}`)}}(e,r?`exon_structure_chr${r.chrom}_${new Date().toISOString().slice(0,10)}`:`exon_structure_${new Date().toISOString().slice(0,10)}`)})}if(i){let t=(i.exonLength/i.totalLength*100).toFixed(1),e=(i.intronLength/i.totalLength*100).toFixed(1);n.innerHTML=`
      ...
      `}},{once:!0}),s.show()}function a(t,e){let o=document.getElementById("exportNotification");o&&o.remove();let n=document.createElement("div");if(n.id="exportNotification",n.className=`export-notification ${t}`,n.innerHTML=`
...
//# sourceMappingURL=exonVisualization.533eda5b.js.map
