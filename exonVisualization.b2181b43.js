(0,globalThis.parcelRequireaed0.register)("iNwrO",function(t,e){function o(t,e,n,a=null){let r=Math.min(...t.map(t=>t.start)),i=Math.max(...t.map(t=>t.end)),s=i-r+1,l={top:45,right:30,bottom:50,left:60},d=e.clientWidth-l.left-l.right,c=d3.select(e).append("svg").attr("width",d+l.left+l.right).attr("height",180+l.top+l.bottom).attr("class","exon-svg").append("g").attr("transform",`translate(${l.left},${l.top})`);a?c.append("text").attr("x",d/2).attr("y",-5).attr("text-anchor","middle").attr("class","chromosome-info text-header").text(`Total Exons: ${t.length} | Chromosome ${a.chrom}${a.strand?`, Strand: ${a.strand}`:""}`):c.append("text").attr("x",d/2).attr("y",-5).attr("text-anchor","middle").attr("class","text-header").style("font-size","14px").style("fill","#666").text(`Total Exons: ${t.length}`);let x=c.append("defs"),p=d3.scaleLinear().domain([r,i]).range([0,d]),m=d3.axisBottom(p).tickFormat(t=>t.toLocaleString()).ticks(Math.min(10,d/100)),h=c.append("g").attr("class","x-axis").attr("transform","translate(0,170)").call(m);h.selectAll("text").style("font-size","10px").style("font-weight","bold"),h.selectAll("line").style("stroke","#ccc"),h.selectAll("path").style("stroke","#ccc").style("stroke-width","2px"),c.append("text").attr("x",d/2).attr("y",180+l.bottom-10).style("text-anchor","middle").style("font-size","12px").style("font-weight","bold").text("Genomic Position"),c.append("g").attr("class","grid-lines").selectAll("line").data(p.ticks(10)).enter().append("line").attr("x1",t=>p(t)).attr("y1",0).attr("x2",t=>p(t)).attr("y2",170).attr("stroke","#eee").attr("stroke-width",1);let u=t.reduce((t,e)=>t+e.length,0),g=[];for(let e=0;e<t.length-1;e++)t[e].end<t[e+1].start&&g.push({start:t[e].end+1,end:t[e+1].start-1,length:t[e+1].start-t[e].end-1,index:e});c.append("line").attr("x1",p(r)).attr("y1",90).attr("x2",p(i)).attr("y2",90).attr("stroke","#ccc").attr("stroke-width",3),x.append("pattern").attr("id","intron-pattern").attr("patternUnits","userSpaceOnUse").attr("width",6).attr("height",6).append("path").attr("d","M-1,1 l2,-2 M0,6 l6,-6 M5,7 l2,-2").attr("stroke","#999").attr("stroke-width",1),c.selectAll(".intron").data(g).enter().append("line").attr("class","intron").attr("x1",t=>p(t.start)).attr("y1",90).attr("x2",t=>p(t.start)).attr("y2",90).attr("stroke","#999").attr("stroke-width",3).attr("stroke-dasharray","5,5").transition().duration(1e3).delay((t,e)=>100*e).attr("x2",t=>p(t.end));let f=[d3.interpolateBlues,d3.interpolateGreens,d3.interpolateOranges,d3.interpolatePurples],b=f[t.length%f.length],v=d3.scaleSequential().domain([0,t.length-1]).interpolator(b),y=c.selectAll(".exon-group").data(t).enter().append("g").attr("class","exon-group").attr("transform",t=>`translate(${p(t.start)}, 70)`);return y.append("rect").attr("class","exon").attr("x",0).attr("y",0).attr("width",0).attr("height",0).attr("fill",(t,e)=>v(e)).attr("stroke","#333").attr("stroke-width",2).attr("rx",6).attr("ry",6).transition().duration(800).delay((t,e)=>100*e).attr("height",40).transition().duration(400).attr("width",t=>Math.max(6,p(t.end)-p(t.start))),y.append("text").attr("class","exon-label").attr("x",t=>Math.max(3,(p(t.end)-p(t.start))/2)).attr("y",25).attr("text-anchor","middle").attr("fill","white").attr("font-weight","bold").attr("font-size","12px").attr("pointer-events","none").style("opacity",0).text((t,e)=>e+1),y.on("mouseover",function(e,o){let n=t.indexOf(o);d3.select(this).select("rect").transition().duration(150).attr("stroke-width",2).attr("stroke","#ff7f0e"),d3.select(this).select(".exon-label").transition().duration(150).style("opacity",1);let a=d3.select("#tooltip");if(a.empty())return void console.warn("Tooltip element not found");a.transition().duration(200).style("opacity",1),a.html(`
                <div class="exon-tooltip">
                    ...
                    <p class="mb-0"><strong>% of Transcript:</strong> ${(o.length/s*100).toFixed(2)}%</p>
                </div>
            `).style("left",`${e.pageX+10}px`).style("top",`${e.pageY-10}px`)}).on("mouseout",function(){d3.select(this).select("rect").transition().duration(150).attr("stroke-width",1).attr("stroke","#333"),d3.select(this).select(".exon-label").transition().duration(150).style("opacity",0);let t=d3.select("#tooltip");t.empty()||t.transition().duration(200).style("opacity",0)}),new ResizeObserver(()=>{if(Math.abs(n.clientWidth-d)>20){let t=e.clientWidth-l.left-l.right;d3.select(e).select("svg").attr("width",t+l.left+l.right),p.range([0,t]),c.select(".x-axis").call(m.ticks(Math.min(10,t/100))),y.attr("transform",t=>`translate(${p(t.start)}, 70)`),y.select("rect").attr("width",t=>Math.max(6,p(t.end)-p(t.start))),y.select("text").attr("x",t=>Math.max(3,(p(t.end)-p(t.start))/2)),c.selectAll(".intron").attr("x1",t=>p(t.start)).attr("x2",t=>p(t.end)),c.selectAll(".text-header").attr("x",t/2)}}).observe(n),{exonCount:t.length,intronCount:g.length,totalLength:s,exonLength:u,intronLength:s-u}}function n(t,e="Node Structure",r=null){let i=document.getElementById("exonVisualizationModal");if(!document.getElementById("tooltip")){let t=document.createElement("div");t.id="tooltip",t.className="exon-tooltip-container",t.style.position="absolute",t.style.opacity="0",t.style.pointerEvents="none",t.style.zIndex="1000",document.body.appendChild(t)}if(i)document.getElementById("exonVisualizationModalLabel").textContent=e;else if((i=document.createElement("div")).id="exonVisualizationModal",i.className="modal fade",i.tabIndex="-1",i.innerHTML=`
...
//# sourceMappingURL=exonVisualization.b2181b43.js.map
